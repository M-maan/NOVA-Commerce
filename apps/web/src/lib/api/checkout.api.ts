...
export type CheckoutSession = { id: string; cartId: string; status: string; currency: string; subtotal: number | string; discountTotal: number | string; shippingTotal: number | string; taxTotal: number | string; grandTotal: number | string; expiresAt: string; shippingMethodId?: string | null; shippingMethod?: { name: string; estimatedDays: number; price: number | string } | null };
